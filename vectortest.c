...
{
  int i;
  const char * const kQuestionWords[] = {"looof","xaxaxaxaxa", "ahdksjahdkasjhdkjwt", "whsadjkahsdere", "hhhhhhow", "why", "heee", "lolslslsl"};
  ...
  for (i = 0; i < kNumQuestionWords; i++) {
    ...
    //VectorAppend(&questionWords, &questionWord);
    if (i > 0)
    {
        printf("i: %d ---- loglen: %d -- ", i, questionWords.logLen);
        PrintString(VectorNth(&questionWords,questionWords.logLen - 1), stdout);
    }
  }
  printf("\n\tLength: %d\n", questionWords.logLen);
  PrintString(VectorNth(&questionWords,questionWords.logLen - 1), stdout);
